"""
This example implements a 2D periodic elasticity simulation using the lattice Boltzmann method (LBM) with multi-grid.
...
if __name__ == "__main__":
    ...
    for i, nx in enumerate(nx_list):
        nx = int(nx)
        ny = nx
        nt = 50 if (nx_list[0] // 2**args.level < 10) else 4 * nx**2
        ...
        if args.plots:
            ...
            if args.save_fig:
                filename = os.path.join(output_dir, f"nx{nx:03d}_l{args.level + i:03d}")
