...
from src.elasticity import LBMElasticity, ElasticityAntiBounceBack
...
class NeumannElasticity(LBMElasticity):
    ...
    def run(self, t_max):
        ...
        self._rel_residual = np.empty((t_max // self.ioRate + 1,))
        ...
        self._l2_eta_exact = np.linalg.norm(self._eta_exact[1:-1, 1:-1].ravel())
        ...
        self._sigma_exact = np.concatenate((sumFunction(self.nx, self.ny, [[], [], [], [2,2]], lambda k,l: np.pi * (-3.2e-3*self.lamda - 3.6e-3*self.mu)),  # sigma xx
                                            ...
                                            axis=-1)
        self._l2_sigma_exact = np.linalg.norm(self._sigma_exact[1:-1, 1:-1].ravel())

        super().run(t_max)

    def get_force(self):
        # epsilon**2 "=" L^2 / U in the non-dimensionalization
        epsilon = 1.0/(self.nx-2)
    ...
    def set_boundary_conditions(self):
        sigma_wall = np.concatenate((sumFunction(self.nx, self.ny, [[], [], [], [2,2]], lambda k,l: np.pi * (-3.2e-3*self.lamda - 3.6e-3*self.mu)),  # sigma xx
                                     sumFunction(self.nx, self.ny, [[2,2], [], [], []], lambda k,l: np.pi * 3.2e-3*self.mu),                         # sigma xy
                                     sumFunction(self.nx, self.ny, [[], [], [], [2,2]], lambda k,l: np.pi * (-3.2e-3*self.lamda - 2.8e-3*self.mu))), # sigma yy
                                     axis=-1)
        ...
        def T_wall(n):
            # epsilon**2 "=" L^2 / U in the non-dimensionalization
            epsilon = 1.0/(self.nx-2)
            return epsilon * np.stack((sigma_wall[..., 0] * n[0] + sigma_wall[..., 1] * n[1], sigma_wall[..., 1] * n[0] + sigma_wall[..., 2] * n[1]), axis=-1)

        # apply anti bounce back boundary condition to the walls
        self.BCs.append(ElasticityAntiBounceBack(tuple(self.boundingBoxIndices["left"].T), self.gridInfo, self.precisionPolicy, T_wall([-1, 0])[-1, 1:-1, ...]))
        self.BCs.append(ElasticityAntiBounceBack(tuple(self.boundingBoxIndices["right"].T), self.gridInfo, self.precisionPolicy, T_wall([1, 0])[0, 1:-1, ...]))
        self.BCs.append(ElasticityAntiBounceBack(tuple(self.boundingBoxIndices["bottom"].T), self.gridInfo, self.precisionPolicy, T_wall([0, -1])[1:-1, -1, ...]))
        self.BCs.append(ElasticityAntiBounceBack(tuple(self.boundingBoxIndices["top"].T), self.gridInfo, self.precisionPolicy, T_wall([0, 1])[1:-1, 0, ...]))

        for level in range(1, self.levels):
            gridInfo = self.grid_info(level)
            bb_indices = self.bounding_box_indices(level)
            self.BCs_MG[level] = [
                ElasticityAntiBounceBack(tuple(bb_indices["left"].T), gridInfo, self.precisionPolicy),
                ElasticityAntiBounceBack(tuple(bb_indices["right"].T), gridInfo, self.precisionPolicy),
                ElasticityAntiBounceBack(tuple(bb_indices["bottom"].T), gridInfo, self.precisionPolicy),
                ElasticityAntiBounceBack(tuple(bb_indices["top"].T), gridInfo, self.precisionPolicy)
            ]
    ...
    def output_data(self, **kwargs):
        ...
        residual = kwargs["f_postcollision"]

        if self.levels > 1:
            if timestep == 0:
                self._init_res = residual
            self._rel_residual[timestep // self.ioRate] = residual / self._init_res

        eta_diff = eta - self._eta_exact
        self._err_l2_eta[timestep // self.ioRate] = np.linalg.norm(eta_diff[1:-1, 1:-1].ravel()) / self._l2_eta_exact
        self._err_lInf_eta[timestep // self.ioRate] = np.linalg.norm(eta_diff[1:-1, 1:-1].ravel(), ord=np.Inf) * (self.nx-2) / self._l2_eta_exact

        sigma_diff = sigma - self._sigma_exact
        self._err_l2_sigma[timestep // self.ioRate] = np.linalg.norm(sigma_diff[1:-1, 1:-1].ravel()) / self._l2_sigma_exact
        self._err_lInf_sigma[timestep // self.ioRate] = np.linalg.norm(sigma_diff[1:-1, 1:-1].ravel(), ord=np.Inf) * (self.nx-2) / self._l2_sigma_exact
...
if __name__ == "__main__":
    ...
    parser.add_argument('-l', '--level', type=int, help="The number of multigrid levels (default: 1).", default=1)
    ...
    output_dir = os.path.abspath("./results/elasticity/neumann")
    ...
    for i, nx in enumerate(nx_list):
        nx = int(nx) + 2
        ny = nx
        nt = 50 if ((nx_list[0]-2) // 2**args.level < 10) else 3 * (nx-2)**2
        ...
        kwargs = {
            ...
            'ny': ny,
            'nz': 0,
            "MG_levels": args.level + i,
            "MG_BCs": {"top": True, "bottom": True, "left": True, "right": True},
            'precision': precision,
        ...
        if args.plots:
            ...
            if args.level > 1:
                pyplot.plot(times, sim._rel_residual, 'k:', label = 'Rel. residual')
