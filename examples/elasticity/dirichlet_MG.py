...
from src.elasticity import LBMElasticity, ElasticityBounceBackFullway
...
class DirichletElasticity(LBMElasticity):
    ...
    def set_boundary_conditions(self):
        eta_wall = np.concatenate((sumFunction(self.nx, self.ny, [[], [2,2], [], []], lambda k,l: 9e-4),
                                   sumFunction(self.nx, self.ny, [[], [], [2,2], []], lambda k,l: 7e-4)), axis=-1)

        # apply bounce back boundary condition to the walls
        self.BCs.append(ElasticityBounceBackFullway(tuple(self.boundingBoxIndices["left"].T), self.gridInfo, self.precisionPolicy, eta_wall[-1, ...]))
        self.BCs.append(ElasticityBounceBackFullway(tuple(self.boundingBoxIndices["right"].T), self.gridInfo, self.precisionPolicy, eta_wall[0, ...]))
        self.BCs.append(ElasticityBounceBackFullway(tuple(self.boundingBoxIndices["bottom"].T), self.gridInfo, self.precisionPolicy, eta_wall[:, -1, ...]))
        self.BCs.append(ElasticityBounceBackFullway(tuple(self.boundingBoxIndices["top"].T), self.gridInfo, self.precisionPolicy, eta_wall[:, 0, ...]))
        ...
        for level in range(1, self.levels):
            ...
            self.BCs_MG[level] = [
                ...
                ElasticityBounceBackFullway(tuple(bb_indices["right"].T), gridInfo, self.precisionPolicy),
                ElasticityBounceBackFullway(tuple(bb_indices["bottom"].T), gridInfo, self.precisionPolicy),
                ElasticityBounceBackFullway(tuple(bb_indices["top"].T), gridInfo, self.precisionPolicy)
            ]
...
if __name__ == "__main__":
    ...
    output_dir = os.path.abspath("./results/elasticity/dirichlet")
    ...
    for i, nx in enumerate(nx_list):
        nx = int(nx) + 2
        ny = nx
        nt = 50 if ((nx_list[0]-2) // 2**args.level < 10) else 4 * (nx-2)**2
        ...
        sim = DirichletElasticity(**kwargs)
        sim.run(nt)
        ...
        if args.plots:
            ...
            pyplot.title(f'E = {E}, nu = {nu}, nx = {nx}, l = {args.level + i}')
            pyplot.show()
