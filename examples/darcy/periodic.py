"""
This example implements a 2D periodic Darcy flow simulation using the lattice Boltzmann method (LBM).
"""
import argparse
import numpy as np
...
class PeriodicDarcy(AdvectionDiffusionReactionBGK):
    ...
    def get_source(self, **kwargs):
        ...
        source = sumFunction(self.nx, self.ny, self._kls, lambda k,l: 0.5 * K * (k**2 + l**2) * np.pi**2 / self.nx**2)
    ...
    def output_data(self, **kwargs):
        ...
        self._l2_p_exact[timestep // self.ioRate] = np.linalg.norm(p_exact.ravel())
        self._lInf_p_exact[timestep // self.ioRate] = np.linalg.norm(p_exact.ravel(), ord=np.Inf)

        p_diff = pressure - p_exact
        self._err_l2_p[timestep // self.ioRate] = np.linalg.norm(p_diff.ravel())
...
if __name__ == "__main__":
    ...
    output_dir = os.path.abspath("./results/darcy/periodic")
